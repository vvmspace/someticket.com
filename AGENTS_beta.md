...
- All cards must be fully clickable (leading to the event page), while buttons lead to ticket ordering.
- **Freshness Guard**: Only future events (not yet passed) must be displayed on the homepage and hub pages.
- **Entity Pruning**: Venues, cities, and hubs without active future events must be hidden from these views.
